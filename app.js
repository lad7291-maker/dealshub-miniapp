...
function renderProducts() {
    ...
    els.resultsInfo.textContent = `Показано ${displayProducts.length} из ${products.length} товаров` + filterText;
...
function createProductCard(product, index) {
    ...
    if (index <= 2) {
        console.log('[createProductCard] index=' + index + ' id=' + product.id + ' title=' + product.title.substring(0,30));
    }
    ...
    return `
        <div class="product-card" data-id="${product.id}" style="animation-delay: ${animationDelay}s">
            ...
            <div class="product-info">
                ...
                ${product.tags && product.tags.length > 0 ? `<div class="product-tags">${product.tags.slice(0,3).map(t => `<span class="product-tag" onclick="event.stopPropagation(); state.searchQuery='${t}'; renderProducts();">${t}</span>`).join('')}</div>` : ''}
...
function openProductModal(id) {
    ...
    console.log('[openProductModal] Found product id=' + product.id + ' title=' + product.title.substring(0,40));
