...
const pwaPromptTsx = fs.readFileSync(
  path.join(v2Dir, 'src', 'components', 'PWAInstallPrompt.tsx'),
  'utf-8'
);
...
test('index.html has apple-mobile-web-app meta tags', () => {
  assert(
    ...
    'apple-mobile-web-app-capable should be present'
  );
  assert(
    indexHtml.includes('name="apple-mobile-web-app-status-bar-style"'),
    'apple-mobile-web-app-status-bar-style should be present'
  );
});
...
test('PWAInstallPrompt.tsx has offline indicator', () => {
  assert(
    pwaPromptTsx.includes('online') || pwaPromptTsx.includes('offline'),
