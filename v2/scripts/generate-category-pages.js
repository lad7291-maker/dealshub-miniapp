...
function generateCategoryHtml(category, products, indexHtml) {
  ...
  const title = category.seoTitle || `Скидки AliExpress на ${category.name} — лучшие предложения | SmartSkidka.ru`
  const description = category.seoDescription || `Лучшие скидки на ${category.name} с AliExpress. Реальные скидки, проверенные отзывы.`
...
function main() {
  ...
  console.log(`✅ Generated ${generated} category pages in ${path.relative(process.cwd(), DIST_DIR)}`)
}
