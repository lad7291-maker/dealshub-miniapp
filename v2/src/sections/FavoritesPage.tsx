...
export function FavoritesPage({
  ...
  const favoriteProducts = useMemo(() => {
    ...
    switch (sortMode) {
      ...
      case 'date':
        return list; // By default order (date added = order in favorites array)
      default:
        return list;
    }
    ...
    return (
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 text-center">
        ...
        <p className="text-sm text-slate-400 max-w-md mx-auto">
          Добавляйте товары в избранное, нажимая на сердечко в карточке товара. Все избранные товары
          сохраняются в вашем браузере.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-white">
            Избранное{' '}
            <span className="text-slate-500 text-base font-normal">({favorites.length})</span>
          </h1>
        </div>
        <div className="flex items-center gap-2">
          {/* Sort */}
          <div className="flex items-center bg-[#1e293b] border border-slate-700/30 rounded-xl overflow-hidden">
            <span className="px-3 text-xs text-slate-500 border-r border-slate-700/30">
              Сортировка
            </span>
            {[
      ...
      <div className="flex items-start gap-2.5 p-3 sm:p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl mb-5 sm:mb-6">
        ...
        <p className="text-xs text-amber-300/80">
          Цены и скидки обновляются ежедневно. Если товар подорожал — скидка может быть уже
          неактуальной. Всегда проверяйте финальную цену на странице AliExpress перед покупкой.
        </p>
      </div>
