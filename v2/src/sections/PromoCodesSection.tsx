...
export function PromoCodesSection({ promos, faq }: PromoCodesSectionProps) {
  ...
  const categoryNames: Record<string, string> = {
    all: 'Все', electronics: 'Электроника', clothing: 'Одежда',
    shoes: 'Обувь', home: 'Дом', auto: 'Авто', sport: 'Спорт', beauty: 'Красота',
  }

  const filtered = categoryFilter === 'all' ? promos : promos.filter(p => p.category === categoryFilter)
    ...
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className="bg-gradient-to-br from-[#1e293b] to-[#1a2636] border border-slate-700/50 rounded-2xl p-5 sm:p-8 mb-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {[
            { step: '1', title: 'Выберите товар', desc: 'Найдите нужный товар в нашем каталоге и перейдите на страницу товара.' },
            { step: '2', title: 'Скопируйте код', desc: 'Нажмите "Копировать" рядом с подходящим промокодом на этой странице.' },
            { step: '3', title: 'Перейдите на AliExpress', desc: 'Нажмите "На AliExpress" в карточке товара и добавьте товар в корзину.' },
            { step: '4', title: 'Примените код', desc: 'При оформлении заказа вставьте промокод в поле "Код купона" и нажмите "Применить".' },
          ].map(item => (
      ...
      {faq && faq.length > 0 && (
        <div className="bg-[#1e293b]/40 border border-slate-700/30 rounded-2xl p-5 sm:p-8">
          <h2 className="text-lg sm:text-xl font-bold text-white mb-5">Частые вопросы о промокодах</h2>
