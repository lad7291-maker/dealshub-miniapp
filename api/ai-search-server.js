...
function checkRateLimit(ip) {
  ...
  return {
    usage,
    remaining: Math.max(0, maxRequests - usage.length),
    exceeded: usage.length >= maxRequests,
  };
}
...
const server = http.createServer((req, res) => {
  ...
  if (isBlocked(ip)) {
    res.writeHead(403);
    res.end(
      JSON.stringify({ error: 'Доступ временно заблокирован. Попробуйте позже.', remaining: 0 })
    );
    return;
  }
  ...
  if (exceeded) {
    ...
    res.end(
      JSON.stringify({
        error: 'Лимит 3 AI-запроса на 24 часа исчерпан. Доступ заблокирован на 1 час.',
        remaining: 0,
      })
    );
    return;
  ...
  logUsage(ip);
  console.log(
    `[AI Search] ${ip} query: "${q.substring(0, 50)}" remaining: ${Math.max(0, remaining - 1)}`
  );
