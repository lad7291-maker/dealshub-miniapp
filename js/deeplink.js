...
(function() {
    ...
    function generateDeeplink(itemId, aliLink, category) {
        ...
        if (itemId && itemId.length > 5) {
            const targetUrl = encodeURIComponent('https://aliexpress.ru/item/' + itemId + '.html?af=' + ADMITAD_ID + '&cv=' + PROGRAM_ID);
            return 'https://s.click.aliexpress.com/deep_link.htm?aff_short_key=' + ADMITAD_KEY + '&dl_target_url=' + targetUrl;
        }
        
        // Fallback: поиск по категории
        const searchUrl = encodeURIComponent('https://aliexpress.ru/wholesale?SearchText=' + encodeURIComponent(category || 'smartskidka'));
        return 'https://s.click.aliexpress.com/deep_link.htm?aff_short_key=' + ADMITAD_KEY + '&dl_target_url=' + searchUrl;
    }

    function generateSearchDeeplink(query, subid) {
        const searchUrl = encodeURIComponent('https://aliexpress.ru/wholesale?SearchText=' + encodeURIComponent(query || 'smartskidka'));
        return 'https://s.click.aliexpress.com/deep_link.htm?aff_short_key=' + ADMITAD_KEY + '&dl_target_url=' + searchUrl;
    }
