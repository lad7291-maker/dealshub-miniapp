...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.INITIAL_CATEGORY && typeof loadCategory === 'function') {
    ...
    els.categoryTabs.forEach((t) =>
      t.classList.toggle('active', t.dataset.category === window.INITIAL_CATEGORY)
    );
  }
  initProducts();
  initSearch();
...
async function renderProducts() {
  ...
  const hasActiveFilters = !!(
    ...
    (state.activeChip && state.activeChip !== 'all') ||
                  ...
                  product.tags && product.tags.length > 0
                    ? '<div class="product-tags">' +
                      product.tags
                        ...
                        .map(
                          (t) =>
                            '<span class="product-tag" onclick="event.stopPropagation(); state.searchQuery=' +
                            escapeHtml(JSON.stringify(t)) +
                            '; renderProducts();">' +
                            escapeHtml(t) +
                            '</span>'
                        )
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (typeof gtag === 'function')
        gtag('event', 'click_outbound', {
          item_id: id,
          value: getProductById(id)?.price || 0,
          currency: 'RUB',
        });
  ...
  els.searchInput.addEventListener('input', (e) => {
    ...
    debounceTimer = setTimeout(async () => {
      ...
      if (query) {
        ...
        if (typeof gtag === 'function')
          gtag('event', 'search', { search_term: query, category: state.currentCategory });
    ...
    tab.addEventListener('click', async () => {
      ...
      if (typeof gtag === 'function')
        gtag('event', 'select_content', {
          ...
          item_id: tab.dataset.category,
        });
      if (typeof ym === 'function')
        ym(109145874, 'reachGoal', 'category', { category: tab.dataset.category });
    });
  });
}
...
function initFilters() {
  els.filterBtns.forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (typeof gtag === 'function')
        gtag('event', 'select_content', { content_type: 'filter', item_id: btn.dataset.filter });
      if (typeof ym === 'function')
        ym(109145874, 'reachGoal', 'filter', { filter: btn.dataset.filter });
    });
  });
}
...
function initSort() {
  els.sortSelect.addEventListener('change', (e) => {
    ...
    // ANA-003: sort event
    if (typeof gtag === 'function')
      gtag('event', 'select_content', { content_type: 'sort', item_id: e.target.value });
...
function openProductModal(id) {
  ...
  console.log(
    '[openProductModal] id=' +
      id +
      ' product=' +
      (product ? product.title.substring(0, 40) : 'NOT FOUND')
  );
  if (!product) return;
